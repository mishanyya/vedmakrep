...
$login=$_SESSION['login_a'];//из сессии мой логин 
...
{
//echo" тип === неизвестен";
exit("<a class='danger' href='adm_vhod2.php'>Тип файла неизвестен.На главную</a>");
}
...
die("<b class='danger'>Запрещённый тип файла</b><a class='danger' href='adm_vhod2.php'>На главную</a>");
// Проверяем размер файла
if ($_FILES['uploadFile']['size'] > $size)

die("<b class='danger'>Слишком большой размер файла</b><a class='danger' href='adm_vhod2.php'>На главную</a>");
...
$upload=htmlspecialchars($upload);
  ...
  $uploadedFile = $foldera.basename($_FILES['uploadFile']['name']); //.basename возвращает имя файла
  ...
  if(is_uploaded_file($_FILES['uploadFile']['tmp_name']))
{
  ...
  {
     echo"<p class='danger'> Файл загружен</p><a class='danger' href='adm_vhod2.php'>На главную</a>";
  }
  else
  {
     echo "<p class='danger'> Во  время загрузки файла произошла ошибка</p><a class='danger' href='adm_vhod2.php'>На главную</a>";
  }
  }
  else
  {
   echo "<p class='danger'>Файл не  загружен</p><a class='danger' href='adm_vhod2.php'>На главную</a>";
  }
  }
...
$size = getimagesize ($foldera.basename($_FILES['uploadFile']['name'])); 
...
$source = imagecreatefromjpeg($foldera.basename($_FILES['uploadFile']['name']));

else if ($_FILES['uploadFile']['type'] == 'image/png')

$source = imagecreatefrompng($foldera.basename($_FILES['uploadFile']['name']));

else if ($_FILES['uploadFile']['type'] == 'image/gif')

$source = imagecreatefromgif($foldera.basename($_FILES['uploadFile']['name']));
...
imagejpeg($dest,$foldera.basename($_FILES['uploadFile']['name']), $quality);
...
if($w_dest<$h_dest){$h_dest=$w_dest;}
  ...
  crop($foldera.basename($_FILES['uploadFile']['name']), $koor_w, $koor_h, $w_dest,$h_dest); // Вызываем функцию обрезать изображение по центру

// переименовать файл в индивидуальное имя

$adres_in=$pdo->prepare("UPDATE admregi_to SET foto='$imyafaila' WHERE login=?");//ввод адреса фотографии в базу данных
$adres_in->execute(array($login));
...
else{echo"<p class='danger'>Файл пока не загружается</p>";}
